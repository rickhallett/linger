...
Supported shell tools expose a string `cmd` or `command`. A recognized shell `-c` argument array can expose its inner string, labelled “shell string from argv”; Input preserves the exact array. Other arrays, dynamic orchestration code, unsupported grammar and parser failures retain their raw input and contextual interpretation route. The first parsed shell unit may leave later multiline content unknown. General nested language exploration, per-token quote/escape explanations and automatic execution-platform matching remain future work.

Literal `cmd`/`command` arguments to `tools.exec_command(...)` and `tools.shell_command(...)` inside `exec`/`functions.exec` are extracted without evaluating JavaScript. Single/double quotes and constant templates are supported, including common escapes. Tab 3 labels the origin; **{ / }** switches between embedded commands in source order. These are separate inspections, not a reconstructed execution sequence. Input retains the full original JavaScript.

This bounded lexical extractor rejects computed/duplicate properties, spreads that could override the command, concatenations and variable arguments. Interpolated templates, regexp syntax and unsupported string escapes may leave a recording unextracted. It does not infer whether a branch ran, resolve variables, parse Python, or execute anything. Each extracted shell string uses the existing local matcher and its coverage limits.
